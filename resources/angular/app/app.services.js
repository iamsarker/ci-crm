...
app.factory('ClientService', function ($q, $filter){
    return {
        setLocalStorage: function (_key, _stringValue) {window.localStorage[_key] = _stringValue;},
        getLocalStorage: function (_key) {
            if (window.localStorage[_key]) {var _resp = window.localStorage[_key];} else {var _resp = "";}
            return _resp;
        },
        ...
        isStrongPassword: function (_str){
            ...
            if( _str.length < 8 ) return "Password length should be 8 characters";
        ...
        clearLocalStorageByKey: function (param) {window.localStorage.removeItem(param);},
        setUser: function (user) {window.localStorage['uSrLoggDIfo'] = JSON.stringify(user);},
        getUser: function () {var _resp = window.localStorage['uSrLoggDIfo'];return JSON.parse(_resp);},
        isLoggedIn: function () {
            var _resp = window.localStorage['uSrLoggDIfo'];
            var usr = JSON.parse(_resp);
            if (usr.user_id !== "") {return true;} else {return false;}
        },
        isRegistered: function () {if (window.localStorage['UserLGOIN']) {return true;} else {return false;}},
        hasPermission: function (module, controller, component) {
            ...
            if(feature===undefined){return false;}else return true;
        },
        ...
        log: function (tag,messaage) {log(tag,messaage);}
    };
});
...
function log(tag,messaage){console.log(tag,messaage);}
