      ...
      <div class="container pd-x-0 pd-lg-x-12 pd-xl-x-0">
        ...
        <div class="row mt-5">
			<div class="col-md-12 col-sm-12">
				<h3 class="d-flex justify-content-between"><span>Expense Vendors</span> <a href="<?=base_url()?>whmazadmin/expense_vendor/manage" class="btn btn-sm btn-secondary"><i class="fa fa-plus-square"></i>&nbsp;Add</a></h3>
				<hr class="mg-5" />
				<nav aria-label="breadcrumb">
					<ol class="breadcrumb breadcrumb-style1 mg-b-0">
						<li class="breadcrumb-item"><a href="<?=base_url()?>whmazadmin/dashboard/index">Portal home</a></li>
						<li class="breadcrumb-item active"><a href="#">Expense Vendors</a></li>
					</ol>
				</nav>
					...
					<?php foreach($results as $row){ ?>
						<tr>
							...
							<td class="text-center">
								...
								<button type="button" class="btn btn-xs btn-danger" onclick="deleteRow('<?=safe_encode($row['id'])?>', <?= json_encode($row['vendor_name'] ?? '') ?>)" title="Delete"><i class="fa fa-trash"></i></button>
							</td>
						</tr>
					<?php } ?>
					</tbody>
