...
class Expense extends WHMAZADMIN_Controller {
		...
		if( $this->input->post() ){
			...
			if ($this->form_validation->run() == true){
				...
				if($this->Expense_model->saveData($form_data)){
					$this->session->set_flashdata('alert', successAlert('Expense has been saved successfully.'));
	...
	public function delete_records($id_val)
	{
		...
		$this->session->set_flashdata('alert', successAlert('Expense has been deleted successfully.'));
