...
class Currency extends WHMAZADMIN_Controller {
		...
		if( $this->input->post() ){
			...
			if ($this->form_validation->run() == true){
				...
				if($this->Currency_model->saveData($form_data)){
					$this->session->set_flashdata('alert', successAlert('Currency data has been saved successfully.'));
	...
	public function delete_records($id_val)
	{
		...
		$this->session->set_flashdata('alert', successAlert('Currency data has been deleted successfully.'));
