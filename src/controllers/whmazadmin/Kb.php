...
class Kb extends WHMAZADMIN_Controller {
		...
		if( $this->input->post() ){
			...
			if ($this->form_validation->run() == true){
				...
				if($this->Kb_model->saveData($form_data)){
					...
					$this->session->set_flashdata('admin_success', 'KB has been saved successfully.');
					redirect("whmazadmin/kb/index");
				}
			} else {
				$this->session->set_flashdata('admin_error', 'Validation error. Try again');
			}
	...
	public function delete_records($id_val)
	{
		...
		$this->session->set_flashdata('admin_success', 'KB has been deleted successfully.');
