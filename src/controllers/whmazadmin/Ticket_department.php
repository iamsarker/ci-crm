...
class Ticket_department extends WHMAZADMIN_Controller {
		...
		if( $this->input->post() ){
			...
			if ($this->form_validation->run() == true){
				...
				if($this->Ticketdepartment_model->saveData($form_data)){
					$this->session->set_flashdata('admin_success', 'Support dept has been saved successfully.');
	...
	public function delete_records($id_val)
	{
		...
		$this->session->set_flashdata('admin_success', 'Support dept has been deleted successfully.');
