...
class Kb_category extends WHMAZADMIN_Controller {
		...
		if( $this->input->post() ){
			...
			if ($this->form_validation->run() == true){
				...
				if($this->Kbcat_model->saveData($form_data)){
					$this->session->set_flashdata('alert', successAlert('KB category has been saved successfully.'));
	...
	public function delete_records($id_val)
	{
		...
		$this->session->set_flashdata('alert', successAlert('KB category has been deleted successfully.'));
