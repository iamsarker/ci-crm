...
class Announcement extends WHMAZADMIN_Controller {
		...
		if( $this->input->post() ){
			...
			if ($this->form_validation->run() == true){
				...
				if($this->Announcement_model->saveData($form_data)){
					$this->session->set_flashdata('alert', successAlert('Announcement has been saved successfully.'));
	...
	public function delete_records($id_val)
	{
		...
		$this->session->set_flashdata('alert', successAlert('Announcement has been deleted successfully.'));
