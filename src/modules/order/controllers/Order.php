...
class Order extends WHMAZ_Controller
{
	...
	{
		$data['summary'] = $this->Billing_model->ticketSummary(getCustomerId())[0];
		$data['results'] = $this->Billing_model->loadInvoiceList(getCustomerId(), -1);

		$this->load->view('invoices_tickets', $data);
	}
	...
	public function newinvoice(){
		...
		$data['user'] = $user;
		$data['recent'] = $this->Billing_model->loadInvoiceList(getCustomerId(), 3);
...
//	{
//            $data['ticket'] = $this->Support_model->viewTicket($tid, getCustomerId());
//            $data['replies'] = $this->Support_model->viewTicketReplies($tid);
//            $data['tid'] = $tid;
//            $this->load->view('support_viewticket', $data);
//	}
//
//
...
//            }
//            redirect("supports/viewticket/".$tid);
//	}
//
...
//                redirect("supports/viewticket/".$ticket_id);
//            }
//
//            $this->form_validation->set_rules('message', 'Message', 'required|trim');
//            $this->form_validation->set_message('message', 'Message is required');
...
//		echo json_encode($this->Support_model->loadTicketList(getCustomerId(), $rqData['limit']));
//	}
//}
