...
- **Encode-for-teeth (IonCube/SourceGuardian):** these anti-piracy checks are plain PHP a buyer can delete, so encode **all four** of: **(1) `src/libraries/License_client.php`** (the check logic + hard-coded admin URL + entitlement client), **(2) `src/controllers/whmazadmin/Authenticate.php`** (the gate call site — without it, a cracker just deletes the two-line call), **(3) `src/libraries/Entitlement.php`** (the entitlement gating library that enforces per-tier feature access), and **(4) `src/helpers/entitlement_helper.php`** (the autoloaded `entitlement_can()` / `entitlement_value()` front-end to it — leaving it plain lets a cracker stub the checks to always-true). The customer download-binding is not anti-piracy (real enforcement would live server-side in `license/verify`), so it needs no encoding. ⚠️ **Before editing any of these four, stop and notify the user** — editing plain source silently desyncs from the encoded build.
