...
⚠️ **Domain renewals are priced LIVE through `Pricing_model::resolve()`; hosting and software renew from their FROZEN `recurring_amount`.** See "Two-Tier Reseller Pricing" — the two halves behave differently per tenant.
...
### Two-Tier Reseller Pricing (v2.0.0 Phase 2)

Resellers buy at a **cost** you set and resell at a **price** they set. Before this there was exactly one price per (product, currency, cycle) and no tiering anywhere in schema or code.

**The three native pricing tables are never written by any of this.** `dom_pricing`, `product_service_pricing` and `software_pricing` still mean **platform retail** and still price direct customers. Everything reseller-specific lives in the new **`price_overrides`** table, keyed `(item_type, pricing_id, owner_company_id, audience)`. Three row shapes cover every tier: `(owner 0, audience 1)` = the platform's default reseller cost, `(owner R, audience 1)` = a negotiated cost, `(owner R, audience 2)` = that reseller's selling price. Migration: `reseller_v2_phase2_migration.sql`. ⚠️ **Run it before the code deploy** — checkout INSERTs `cost_amount` and dies on `Unknown column` otherwise.
...
2. **`R == 0` → sell = base, cost = 0, return.** ← this short circuit is why the refactor is provably a no-op for direct customers; they never read `price_overrides` at all.
3. `cost` = `override(R, COST)` ?? `override(0, COST)` ?? `reseller_profiles` discount applied to base ?? base.
4. `B` **is** the reseller → they pay cost.
5. `B` is a sub-customer → `override(R, RETAIL)` ?? **base** (never cost — an unpriced item must not be sold at wholesale).

Domains resolve **register / transfer / renewal independently**; services and software mirror their single figure onto all three so callers need no branching. `resolveMany()` batches the TLD grid into one query per audience (negative-cached, so unpriced extensions cost nothing). Request-cached, **never session-cached**.

**Every price read funnels through it** and each call site **keeps its return shape** — `item_price`/`price`/`transfer`/`renewal` are overwritten in place, so `Cart.php`, the Angular controllers and the API responses needed no change: `Cart_model` (`getDomPricing`, `getCartDomainPrice`, `getCartServicePrice`, `getCartSoftwarePrice`, `getDomPricingById`, `getProductServicePricingById`, `getProductServiceItems`), `Common_model` (`getDomainPrices`, `getHostingPrices`), `ApiDomains::_domPricing`, `ApiProducts`. Each gained an optional `$companyId`; the API mirrors pass it explicitly via **`API_Controller::pricingCompanyId()`** (defaults to the reseller's own company = their cost; `customer_id` asks what that customer would be quoted) because catalog endpoints are stateless. Order placement needs none of this — `actAsCustomer()` sets a CUSTOMER session and `getCompanyId()` picks it up.

**Price floor — `Pricing_model::saveResellerRetail()`, server-side and PER COMPONENT.** The JS hint is decoration. Per component because a blended floor lets a reseller price registration above cost and renewal below it, and the renewal is the one that repeats for the life of the domain. A blank transfer/renewal inherits `price`, so `price` alone must clear all three floors. `resolve()` also clamps `max(sell, cost)` on read, covering the window between a cost rise and the auto-lift.

**Auto-lift — the half nobody thinks about.** Raising a cost strands existing retail overrides below the new floor. `liftUnderwaterRetail()` pulls them up (`price = GREATEST(price, new_cost)`), writes a `price_override_audits` row, and `notifyLiftedResellers()` emails them (`reseller_price_lifted` template, with a hardcoded fallback body so a missing row degrades to a plainer email rather than silence). Raising the platform-wide cost (owner 0) affects every reseller without their own negotiated cost; raising one reseller's affects only them.

**Renewal: domains are LIVE, hosting and software are FROZEN.** Support will be asked about this.
- **Hosting/software** renew from `order_services.recurring_amount` / `order_licenses.recurring_amount` as they always did. No change.
- **Domains** route through the resolver. `getExpiringDomains()` used to take `dp.renewal` — the *platform's* renewal price — for a reseller's customer, a straight revenue leak. The `dom_pricing` join **stays as a filter** (it is what requires a price row in the order's currency, and it supplies `de.extension`); only the price it produced is replaced. Live is right here: registrar wholesale moves must pass through, and both parties expect a price change to apply at the next renewal.

**Cost snapshots.** `cost_amount` on `order_domains` / `order_services` / `order_licenses`, frozen at checkout by `Cart::_resolveCostAmount()` (and by the admin new-order path) alongside the sell price. Phase 3 debits the wallet from it, potentially days later; recomputing then would let a price change silently rewrite what the reseller is billed. Transfers freeze the transfer cost; `dns_update` domains and imported (`order_type` 3/4) items cost 0.00. **Domain renewals re-freeze it** — a renewal does not cost what the registration cost, and leaving the original would debit a first-year price forever.

**Admin UI.** Domain Pricing gains a "Reseller Cost" section; the hosting and software pricing matrices gain a shaded cost input under each retail cell (a blank cell **clears** the cost — it is not a zero, which would mean giving the item away). New **`whmazadmin/reseller_pricing`** serves both audiences: a reseller edits their own prices with cost read-only beside them, a platform admin picks a reseller and can also set that reseller's negotiated cost. The controller **pins a reseller admin to their own company and ignores `?reseller=`** — the capability hook knows class+method but not which company an id names.
...
**Also fixed here:** `Cart::addToCartAjax()` charged `dom_pricing.price` for transfers — only `linkDomainToHosting()` and `addDomainToCart()` branched on the action. A live billing bug, and a two-tier money bug waiting to happen.
