...
- **Encode-for-teeth (IonCube/SourceGuardian):** these anti-piracy checks are plain PHP a buyer can delete, so encode **all six** of: **(1) `src/libraries/License_client.php`** (the check logic + hard-coded admin URL + entitlement client), **(2) `src/controllers/whmazadmin/Authenticate.php`** (the gate call site — without it, a cracker just deletes the two-line call), **(3) `src/helpers/entitlement_helper.php`** (the autoloaded `entitlement_can()` / `entitlement_value()` front-end — leaving it plain lets a cracker stub the checks to always-true), **(4) `src/libraries/Entitlement.php`** (the entitlement gating library that enforces per-tier feature access), **(5) `src/views/templates/customer/footer.php`** and **(6) `src/views/whmazadmin/include/footer.php`** (the customer + admin branding-removal enforcement points — `if(!feature_enabled('branding_removal'))` wraps the "Maintain by WHMAZ" backlink in each; branding is presentational-only with no server-side backstop, so deleting either plaintext call site IS the bypass. Encode both or the backlink stays removable from whichever is left plaintext. Mixed HTML/PHP views: IonCube encodes them fine but you lose easy template edits — edit the unencoded copy, then re-encode). The customer download-binding is not anti-piracy (real enforcement would live server-side in `license/verify`), so it needs no encoding. Canonical list with full rationale: **`ioncube-encodes-files.txt`** (repo root). ⚠️ **Before editing any of these six, stop and notify the user** — editing plain source silently desyncs from the encoded build.
